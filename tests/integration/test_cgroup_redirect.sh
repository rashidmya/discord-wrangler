...
if ! command -v microsocks >/dev/null 2>&1; then
    echo "SKIP: microsocks not installed"; exit 0
fi
if ! command -v systemd-run >/dev/null 2>&1; then
    echo "SKIP: systemd-run required"; exit 0
fi
if ! command -v ncat >/dev/null 2>&1; then
    echo "SKIP: ncat required (install nmap-ncat / ncat)"; exit 0
fi
...
# Loopback echo server (listens on 127.0.0.1; cgroup TCP from inside the
# scope will be redirected here regardless of stated destination, because
# the nft rule fires on cgroup membership, not destination).
ncat -l -k -p $ECHO_PORT --exec "/bin/cat" -s 127.0.0.1 &
ECHO_PID=$!
...
# Start the daemon (it installs the rule once the launcher creates the scope).
...
# Launch a TCP client inside the scope. The launcher script lives in share/.
chmod +x "$ROOT/share/discord-wrangler-launch"
DISCORD_BIN=/bin/sh \
    sudo -u "$USERNAME" XDG_RUNTIME_DIR=/run/user/$USERID \
    "$ROOT/share/discord-wrangler-launch" \
    -c "echo hello | ncat -w 2 127.0.0.1 $ECHO_PORT" > /tmp/cgroup_redirect_result 2>&1 || true
...
# our echo server). The echo server echoes "hello" back. We expect "hello"
# to appear in the captured output.
